$(function() {
	...
	$("#edit").click(function() {
		...
		$(".del").click(function() {
			...
			$(this).parent().parent().slideUp(400, function() {
				this.remove()
			}); //parent is <p> then grand-parent is <div class="thing">
	...
	$("#flickrGet").click(function() {
		...
		var mytags = "";
		s.each(function() {
			mytags += " " + $(this).attr("data-tags")
		}); //grab all tags as string (seperated by spaces)
		...
		var theFilterArray = []; //init filter array as empty...

		$(".tagfilter").click(function() {

			theFilterText = $(this).text();
			theFilterArray.push(theFilterText);
			performFilter();
			...
			$(".filter-close-x").last().click(function() { //note select last - one just added

				//remove filter text from the filter array:

				var removeText = $(this).prev(".filter-text").text();
				theFilterArray = _.without(theFilterArray, removeText);
				performFilter();
		...
		}); //end tag filter click handler


		function performFilter() { // changed now to use theFilterArray - captured variable

			var s = $('#flickrfeed >.flickrFeedItem');

			s.each(function() {

				var thisTagsArray = $(this).attr("data-tags").trim().split(/\s+/); //clean up the string and make into an array


				if ((_.intersection(thisTagsArray, theFilterArray).length > 0) || (theFilterArray.length == 0)) {
